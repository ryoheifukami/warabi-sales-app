...
# ===== Streamlit 画面 =====
def read_secret_key():
    """Secretsから APIキーを読む。名前のゆらぎ・前後空白・余分なクォート・改行に強くする。"""
    try:
...
def main():
    ...
    api_key = secret_key  # APIキーは管理者がSecretsに設定。利用者は触らない
    ...
    with st.sidebar:
        ...
        with st.expander("管理者向け診断", expanded=False):
            if api_key:
                masked = f"{api_key[:12]}…{api_key[-4:]}" if len(api_key) > 16 else "（短すぎます）"
                st.caption(f"APIキー検出：長さ {len(api_key)} 文字 / {masked}")
                if not api_key.startswith("sk-ant-"):
                    st.warning("キーが『sk-ant-』で始まっていません。Anthropic以外のキーかもしれません。")
                else:
                    st.caption("形式OK（sk-ant- で始まっています）。401が続く場合はキーの有効性・残高を確認してください。")
            else:
                st.caption("APIキー：未検出（Secrets に ANTHROPIC_API_KEY を設定してください）")

    if not api_key:
        st.error("ただいまご利用いただけません。管理者にお問い合わせください。")
        st.caption("（管理者向け：Streamlit の Settings → Secrets に ANTHROPIC_API_KEY を設定してください）")
        st.stop()
