...
必要な secrets（Streamlit Cloud の Settings → Secrets に設定）:
    ANTHROPIC_API_KEY = "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"
    APP_PASSWORD      = "warabi"
"""
...
def main():
    ...
    if not api_key:
        st.error("APIキーが設定されていません。管理者に連絡してください（Secrets の ANTHROPIC_API_KEY）。")
        st.stop()
    ...
    st.subheader("① URLを貼る（1行に1社）")
    urls_text = st.text_area("お客様のサイトURL", height=160,
